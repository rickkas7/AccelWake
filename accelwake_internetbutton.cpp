...
// The Internet Button internal connections for its ADXL362 are:
// SCL: A3 (SCK)
// SDA: A5 (MOSI)
// SDO: A4 (MISO)
// CS: A2 (SS)
...
// This is the pin connected to the INT1 pin; the InternetButton uses A7/WKP
const int ACCEL_INT_PIN = A7;
...
void loop() {
	...
	case PUBLISH_STATE:
		if (Particle.connected()) {
			...
			// cell voltage (decimal) - mot supported on button without power shield
			// state of charge (decimal) - mot supported on button without power shield
			char data[32];
			snprintf(data, sizeof(data), "%d,0,0", awake);
